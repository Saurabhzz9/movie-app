...
export class Tab1Page implements OnInit {
  ...
  ngOnInit(): void {
    this.initializerSliderContainer();
    this.initializeGenreContainer();
    this.initializeContainer();
}
initializerSliderContainer(){
  this.service.getTrendingList(this.modelType).subscribe(trendingMoviesEl => {
    trendingMoviesEl.results.forEach(trendingMovies => {
      this.sliderContainer.push({
         id:trendingMovies.id,
         title: trendingMovies.title,
         image: ('http://image.tmdb.org/t/p/original/' + trendingMovies.backdrop_path),
         posterPath: ('http://image.tmdb.org/t/p/original/' + trendingMovies.poster_path),
